...
(function($) {
	$.fn.autogrow = function(options) {
		var defaults = {
			...
			enterToSubmit : false,
			autoCollapse : true
		/* false to have better performance */
		};
		...
		function resize(e, opts) {
			...
			contentLength = $e.val().length, elementWidth = $e.innerWidth();
			...
			if (contentLength != $e.data("autogrow-length") || elementWidth != $e.data("autogrow-width")) {
				...
				if (hCheck && (contentLength < $e.data("autogrow-length") || elementWidth != $e.data("autogrow-width"))) {
					$e.css("height", "0px");
				}

				var extraLines = opts.extraLines ? opts.extraLines : 0;
				var height = Math.max($e.data("autogrow-min"), Math.ceil(Math.min($e.attr("scrollHeight") + (options.expandTolerance + extraLines)
						* $e.data("autogrow-line-height"), $e.data("autogrow-max"))));
