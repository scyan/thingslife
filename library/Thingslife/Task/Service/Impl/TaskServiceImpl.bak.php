...
class TaskServiceImpl extends BaseService implements ITaskService {
	...
	public function archive(array $conditions){
	    ...
	    $this->getTaskDao()->multiUpdate(array('focusType'=>'archived','focus'=>'archived'),$conditions);
	}
	...
	public function moveToFocusType($focusType, $task) {
 	...
 	public function destroy($ids){
		$builder=$this->getTaskDao()->deleteBuilder();
		if($ids){
		    $builder->condition('_id',implode(',',$ids),'IN');
		}
			$builder->condition('focusType','deleted');
...
/* 	    $builder->condition('userId',$userId);
	...
	public function listArchived($offset,$limit,array $conditions=array()) {
	    var_dump($offset);
	    var_dump($limit);
		$builder = $this->getTaskDao ()->queryBuilder ();
		 if(!isset($conditions['userId'])){
    ...
    public function loadTag($id){
     ...
     public function addTag($taskId,$data){
	       $data=base64_decode($data);
    		$name=md5($taskId.time().rand()).'.png';
    	   $im = imagecreatefromstring($data);
           if ($im !== false) {
               header('Content-Type: image/png');
               //TODO notice 修改tag name表示方式
              // imagepng($im,$_SERVER['TAG_FOLDER'].'tag_'.$taskId.'.png');
               imagepng($im,$_SERVER['TAG_FOLDER'].$name);
               imagedestroy($im);
           }
	   return $this->getTagsDao()->add(array('taskId'=>$taskId,'name'=>$name));
	} 
/* 	public function addTag($user,$data){
	    $data=base64_decode($data);
	     
	    $im = imagecreatefromstring($data);
	    $name=md5(`{$user->id}`.time().rand()).'png';
	    if ($im !== false) {
	    	header('Content-Type: image/png');
	    	imagepng($im,$_SERVER['TAG_FOLDER'].$name);
	    	imagedestroy($im);
	    }
	    return $this->getTagsDao()->add(array('name'=>$name));
	} */
	public function deleteTag($tagId){
	    $tag=$this->loadTag($tagId);
	    if(file_exists($_SERVER['TAG_FOLDER'].$tag['name'])){
	    	unlink($_SERVER['TAG_FOLDER'].$tag['name']);
	    }
	    return $this->getTagsDao()->delete($tagId);
	}
	public function deleteTagByTaskId($taskId){
	     $filename='tag_'.$taskId.'.png';
         if (file_exists($_SERVER['TAG_FOLDER'] .$filename)){
             unlink($_SERVER['TAG_FOLDER'].$filename);
         }
	  return $this->getTagsDao()->deleteByTaskId($taskId);
	}
	public function updateTag($tagId,$taskId,$data){
	    $tag=$this->loadTag($tagId);
	    //删除旧文件
	    if (file_exists($_SERVER['TAG_FOLDER'] .$tag['name'])){
	    	unlink($_SERVER['TAG_FOLDER'].$tag['name']);
	    ...
	    $this->getTagsDao()->update(array('name'=>$name),$tagId);
	}
	...
	public function createTaskFromRepeat($today) {
		...
		foreach ( $result as $repeat ) {
			if ($repeat ['next'] == $today) {
				...
				$newTagName='tag_'.$newTaskId.'png';
				copy($tag_floder.'tag_'.$repeat ['taskId'] .'png', $tag_floder.$newTagName);
				$tagid=$this->getTagsDao()->add(array('taskId'=>$newTaskId,'name'=>$newTagName));
				
				$this->getTaskDao ()->update ( array ('instance' => $newTaskId ), $templateId );
				$this->getTagsDao()->update(array('tag'=>$tagid),$newTaskId);
			}
		}
	}
